...
/**
 * Classe Jogadores no pacote view, um CRUD de adicionar novos jogadores,
 * podendo escolher o nome, a posicao e o time.
 * 
...
public class Jogadores {
	 ...
	 * alem de estilizar e adicionar em Jogadores.
	 */
	...
	/**
	 * Painel com os jogadores em ordem alfabetica
	 */
	...
	private void metodos() {
		...
		/** Ordena os jogadores em ordem alfa de time. */
	...
	/**
	 * JPanel hub e um container de conteudo e o uso de GridBagLayout foi para
	 * posicionar os elementos que pertencem ao hub.
	 */

	public JPanel hub() {
		JPanel HUB = new JPanel(new GridBagLayout());
		HUB.add(new JLabel("ID", SwingConstants.CENTER),
				posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 1, 0, 1, 1));
		HUB.add(ID, posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 1, 1, 1, 1));
		HUB.add(Adicionar, posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 2, 1, 1, 1));
		HUB.add(new JLabel("Nome", SwingConstants.CENTER),
				posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 1, 2, 2, 1));
		HUB.add(Nome, posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 1, 3, 2, 1));
		HUB.add(new JLabel("Posicao", SwingConstants.CENTER),
				posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 1, 4, 2, 1));
		HUB.add(Posicao, posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 1, 5, 2, 1));
		HUB.add(new JLabel("Clube", SwingConstants.CENTER),
				posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 1, 6, 2, 1));
		HUB.add(Clube, posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 1, 7, 2, 1));
		HUB.add(Finalizar, posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 1, 8, 2, 1));
		HUB.add(Deletar, posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 1, 9, 2, 1));
		HUB.add(new JLabel("Jogadores", SwingConstants.CENTER),
				posComponentes(GridBagConstraints.HORIZONTAL, 1, 0, 0, 0, 1, 1));
		HUB.add(new JScrollPane(ViewJogadores), posComponentes(GridBagConstraints.BOTH, 1, 0, 0, 1, 1, 10));
		return HUB;
	}

	/**
	 * O metodo acoes adiciona alguns acontecimentos ao presionar os butoes, como
	 * adicionar, finalizar e deleter.
	 */
	...
	private void acoes() {
		...
		Finalizar.addActionListener(e -> {
			if (!Nome.getText().equals("")) {
				...
				/**
				 * se o id estiver marcando um numero igual ao index (controle de jogadores
				 * criado) � caso de adicionar.
				 */
				if (ID.getText().equals(String.valueOf(Index))) {
					...
					if (Menu.brasileirao.addJogador(idTimeNovo, Nome.getText(), Posicao.getSelectedItem().toString())) {
						...
						JOptionPane.showMessageDialog(Frame, "Cadastro de jogador realizado com sucesso.");
					} else {
						JOptionPane.showMessageDialog(Frame, "Algo de errado aconteceu com o seu cadastro.");
					}
				} else {
					/** Se n�o for caso de adicionar, faz-se o update. */
					...
					if (idTimeOriginal != idTimeNovo) { /**
														 * id do time original e do time novo s�o diferentes -> troca de
														 * times.
														 */
						jogEmUpdate.setIdTime(idTimeNovo);
						jogEmUpdate.setTime();
						Campeonato.times.get(posNovoTimeEmTimes).getJogadores().add(jogEmUpdate);
						int posJogNaListaDeJogDoTimeOriginal = posJogEmUmaLista(ID.getText(), Campeonato.times
								.get(Menu.brasileirao.posTimeNaListaDeTimes(idTimeOriginal)).getJogadores());
						...
						int idIndexTime = posJogEmUmaLista(ID.getText(),
								Campeonato.times.get(posNovoTimeEmTimes).getJogadores());
		...
		Deletar.addActionListener(e -> {
			...
			if (Menu.brasileirao.deleteJogador(posTimeNaListaTimes, Selecionado.getIdJog())) {
				JOptionPane.showMessageDialog(Frame, "Jogador excluido com sucesso.");
			} else {
				JOptionPane.showMessageDialog(Frame, "Erro ao excluir jogador.");
			}
			metodos();
		});
	}
